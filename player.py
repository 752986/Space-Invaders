...
class Player(Sprite):
    ...
    def on_projectile_deleted(self, event: Event):
        assert type(event) is projectile.ProjectileDeleted # this is needed to appease the type checker
    ...
    def __init__(self, game_state: GameState, pos: pygame.Vector2):
        ...
        self.can_shoot = True
        game_state.event_handler.subscribe(self, projectile.ProjectileDeleted, self.on_projectile_deleted)
    ...
    def update(self, game_state: GameState, delta: float):
        ...
        if keys[keymap.FIRE] and self.can_shoot:
            game_state.game_objects.append(projectile.Projectile(game_state, pygame.Vector2(0, -1000), self))
